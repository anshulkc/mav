            ...
            if (wantsStream) {
                return signedStreamSSE(c, privateKey, config, async (stream) => {
                    try {
                        ...
                        console.log(`Executing tool ${tool.id} in streaming mode${withContext ? ' with context' : ''}`);
                        const result = await tool.handler({ ...body, DAIN_EXTRA_DATA: undefined }, agentInfo, {
                            ...
                            updateUI: async (update) => {
                                try {
                                    ...
                                    console.error(`Error sending UI update in ${tool.id}:`, error);
                                }
                            },
                            ...
                            try {
                                const contextPromises = contexts.map(async (context) => {
                                    try {
                                        ...
                                        console.error(`Error getting context ${context.id}:`, contextError);
                                        // Return a partial context object with error information
                                        return {
                                            id: context.id,
                                            name: context.name,
                                            description: context.description,
                                            error: contextError.message || "Error fetching context data",
                                        };
                                    }
                                });
                                ...
                                console.error(`Error gathering contexts for ${tool.id}:`, contextError);
                        ...
                        console.log(`Sending final ${withContext ? 'context ' : ''}result for ${tool.id}`);
                        await stream.writeSSE({
                            event: 'result',
                            data: JSON.stringify(processedResult),
                            id: Date.now().toString(),
                        });
                        console.log(`Final result sent for ${tool.id}`);
                    }
                    catch (error) {
                        ...
                        try {
                            ...
                            console.error(`Failed to send error result:`, streamError);
                        }
                        throw error;
                    }
                });
                ...
                try {
                    ...
                    if (withContext) {
                        try {
                            ...
                            console.error(`Error gathering contexts for ${tool.id}:`, contextError);
                            // Return result with empty contexts if context gathering fails
                            response = {
                                toolResult: result,
                                context: [],
                            };
                    ...
                    // This ensures client code can properly handle the error
